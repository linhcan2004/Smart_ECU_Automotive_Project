...
│
│
├── MProject_ECU/                   <-- 1. THƯ MỤC DỰ ÁN NHÚNG STM32 (Trong STM32CubeIDE)
│   │
...
│   │   │
│   │   ├── Src/
│   │   │   ├── main.c              <-- (Mặc định) File gốc khởi động chip (Thành viên 1 cấu hình tuần 1)
...
│   │       │   └── app_bsp.h       <-- Chỉ cần file .h: Định nghĩa Struct xe, khóa Mutex, hàng đợi Queue
│   │       │
...
│   │       │   ├── mpu6500.c       <-- Viết code thô giao tiếp đọc dữ liệu SPI/I2C từ chip MPU6500
│   │       │   ├── safety_mgr.h    <-- Khai báo hàm giám sát an toàn và Watchdog
│   │       │   └── safety_mgr.c    <-- Viết logic chống lật xe và kiểm tra chu kỳ sống để cho chó ăn (Watchdog)
│   │       │
...
│   │       │   └── flash_mem.c     <-- Viết code đọc/ghi số VIN, số Km tích lũy vào Flash của chip
│   │       │
...
│   │           ├── control_pid.c   <-- Viết công thức toán PID tính toán xung PWM xuất ra động cơ L298N
│   │           ├── encoder.h       <-- Khai báo hàm đọc xung bánh xe
│   │           ├── encoder.c       <-- Viết code đọc Timer ở chế độ Encoder Mode để tính vận tốc thực
│   │           ├── line_sensor.h   <-- Khai báo hàm đọc mắt hồng ngoại
│   │           └── line_sensor.c   <-- Viết code đọc trạng thái dãy 5 cảm biến hồng ngoại bám vạch
│   │
│   └── MProject_ECU.ioc            <-- (Mặc định) File cấu hình cấu hình chân chip giao diện đồ họa
│
│
└── Host_PC_TesterTool/             <-- 2. THƯ MỤC PHẦN MỀM PYTHON (Chạy trên máy tính)
    ...
    │   └── bluetooth_mgr.py        <-- Lập trình luồng chạy ngầm kết nối Bluetooth Socket RFCOMM với xe
    └── Protocols/                  <-- Thư mục chứa code mã hóa giao thức
        ├── uds_encoder.py          <-- Dịch nút bấm thành các chuỗi mã byte UDS (0x10, 0x27...)
        └── uds_decoder.py          <-- Giải mã gói tin từ xe gửi lên và thuật toán Seed & Key phía PC
